...
public:
  ...
  // It's weird not using `pyosg` here, but doing so would break
  // serialization.
...
public:
  ...
  // unsigned int getUserObjectIndex(const osg::Object* obj, unsigned int
  // startPos=0) const = 0; unsigned int getUserObjectIndex(const std::string&
  // name, unsigned int startPos=0) const = 0; void setDescriptions(const
  // DescriptionList& descriptions) = 0; DescriptionList& getDescriptions() = 0;
  ...
  unsigned int getNumUserObjects() const override {
    PYBIND11_OVERRIDE_PURE(unsigned int, osg::UserDataContainer,
                           getNumUserObjects);
  }

  // const Object* getUserObject(unsigned int i) const  = 0;
  osg::Object *getUserObject(unsigned int i) override {
    PYBIND11_OVERRIDE_PURE(osg::Object *, osg::UserDataContainer,
                           getUserObject i);
  }
};
} // namespace detail
