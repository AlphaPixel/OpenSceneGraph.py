...
- `osg.Vec3(...)` constructs a `Vec3f`, not a `Vec3d` — `Vec3`/`Vec2`/`Vec4`
  are just aliases for the float variant (`bind_alias_Vec<osg::Vec3f, 3>(m,
  "Vec3f", "Vec3")` in `pyosg/osg/Vec.hpp`). A handful of real OSG APIs are
  `Vec3d`-explicit with **no** implicit Vec3f→Vec3d conversion registered
  anywhere (confirmed by reading `Vec.hpp`'s shared `bind_Vec<T, N>` template
  — only same-type copy constructors exist): `osg.Plane`'s `(normal,
  distance)` and `(a, b, c, d)` overloads, and
  `PositionAttitudeTransform`/`AutoTransform`'s `position`/`scale`/
  `pivotPoint` (both the kwargs constructor and the setters — see
  `Transform.cpp`'s own comment on this). Passing a plain `osg.Vec3(...)` to
  any of these raises a `TypeError: incompatible constructor arguments` (for
  `Plane`) or a cast failure (for the `Transform` kwargs/setters) — use
  `osg.Vec3d(...)` explicitly instead. `Quat`'s constructors are NOT in this
  category — it genuinely registers both `Vec3f` and `Vec3d` overloads.
