...
| [`12-gbuffer.md`](12-gbuffer.md) | Designing a generic G-buffer or composing deferred/NPR post-process passes from canonical color, normal, and depth data. Start from `examples/pyosg-gbuffer.py`. |
| [`13-depth-readback.md`](13-depth-readback.md) | CPU depth readback + `osgx.unprojectPoint()` world-position reconstruction (the depth sibling of picking). **Read before adding any debug/decal geometry positioned FROM that readback** — placing it in the same pass the readback samples, without disabling depth WRITE on it, closes a real feedback loop (looks like a race; isn't). |
