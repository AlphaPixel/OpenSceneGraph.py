...
| [`10-rtt.md`](10-rtt.md) | Building a render-to-texture / multi-camera scene graph live (RTT cameras, `osg.Camera()` construction quirks, `Group.children` limitations). **Read before debugging any fullscreen pass that outputs one flat color** -- fullscreen quads must disable `GL_DEPTH_TEST` or they break silently the moment they're re-targeted to an FBO. |
